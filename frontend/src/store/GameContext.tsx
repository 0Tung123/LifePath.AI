...
export const GameProvider: React.FC<{children: ReactNode}> = ({ children }) => {
  ...
  const createCharacter = async (data: CharacterCreateInput): Promise<Character> => {
    try {
      setLoadingCharacters(true);
  ...
  const generateCharacter = async (data: CharacterGenerateInput): Promise<Character> => {
    try {
      setLoadingCharacters(true);
  ...
  const makeChoice = async (choiceId: string): Promise<void> => {
    ...
    try {
      setLoadingSession(true);
      const updatedSession = await gameApi.makeChoice(currentSession.id, choiceId);
  ...
  const processUserInput = async (inputData: UserInputData): Promise<void> => {
    ...
    try {
      setLoadingSession(true);
      const updatedSession = await gameApi.processUserInput(currentSession.id, inputData);
      setCurrentSession(updatedSession);
